...
def augment(image, non_picked_list):
    ...
    # Compute actual patch size in px: division by 1.5 since we multiplied by 1.5 in crop_large_patch. Division by 2 to get the middle
...
def crop_large_patch(folder_path, file_name, mean_std_dict):
    ...
    # Compute patch size. Final patch: (1/7) * 2 width, (1/8) * 2 height. To augment (particularly for rotation), we need a slighty bigger patch, so x1.5
    patch_size = np.array([math.floor(1.5 * 1.0/7.0 * width), math.floor(1.5 * 1.0/8.0 * height)])
