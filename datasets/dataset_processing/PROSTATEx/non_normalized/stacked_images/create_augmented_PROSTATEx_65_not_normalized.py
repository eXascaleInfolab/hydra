...
def augment(stacked_image, non_picked_list):
    ...
    temp_stacked_image = [img.crop((x_middle - 33 + shifting, y_middle - 33, x_middle + 32 + shifting, y_middle + 32)) for img in temp_stacked_image]
    ...
    temp_stacked_image = np.array([ np.array( img ) for img in temp_stacked_image ])
            ...
            for t2_file_name in sequences_to_filenames['t2']:
                for dwi_file_name in sequences_to_filenames['dwi']:
                    for adc_file_name in sequences_to_filenames['adc']:
                        ...
                        stacked = align_stack(input_folder, t2_file_name, dwi_file_name, adc_file_name, mean_std_dict)
                        ...
                        for augmented_index in range(nb_augmented_images):
                            ...
                            # Export the image
                            output_name = output_folder/ f"t2-{'_'.join(t2_file_name.stem.split('_')[0:3])}_dwi-{'_'.join(dwi_file_name.stem.split('_')[2:3])}_adc-{'_'.join(adc_file_name.stem.split('_')[2:3])}_visit{visit}_augmented-{augmented_index}.npy"
