...
def train_model(model, train_data_loader, learning_rate, nb_epoch, loss_function, optimizer, device, writer, tensorboard_output_name, optimized_metric, validation_data_loader=None, save_intermediate=''):
    ...
    if validation_data_loader != None:
        ...
        scheduler = ReduceLROnPlateau(optimizer, 'min', verbose=True, patience=2, factor=0.01 )
        # Best model
    ...
    for epoch in range(nb_epoch):
        ...
        if(validation_data_loader != None):
            ...
            # Decrease the learning rate when the training loss increases
            scheduler.step(metrics_epoch_val['auc'])
...
if __name__ == "__main__":
    ...
    paths_dict={'training': args.trainingset, 'validation': args.validationset},
    fixed_parameters_dict={'loss_function': loss_functions_dict[args.lossfunction], 'cuda_device': args.cudadevice, 'dropout': args.dropout, 'optimized_metric': args.optimizedmetric},
